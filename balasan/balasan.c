...
void balasKicauan(int idkic,int idbal ,List *l){
    ...
    }else if(isPenggunaPrivate(&LPengguna, getElmt(*l,indexOf(*l,idkic)).author)  && !isTeman(IdCurrentUser, findIDPenggunaByName(&LPengguna,getElmt(*l,indexOf(*l,idkic)).author))){
        printf("\nWah, akun tersebut merupakan akun private dan anda belum berteman dengan akun tersebut!\n");
    }else if(idbal != -1 && isPenggunaPrivate(&LPengguna,BALASAN(findBalasanInTree(getElmt(*l,indexOf(*l,idkic)).Balas,idbal)).writer) && !isTeman(IdCurrentUser, findIDPenggunaByName(&LPengguna,BALASAN(findBalasanInTree(getElmt(*l,indexOf(*l,idkic)).Balas,idbal)).writer))){
        printf("\nWah, akun tersebut merupakan akun private dan anda belum berteman dengan akun tersebut!\n");
    }else{
        printf("\nMasukkan balasan : \n");
        Word text;
...
void printAllBalasanWithIndent(Tree T, int depth) {
    ...
    if(depth != 0){
        if(isPenggunaPrivate(&LPengguna,BALASAN(T).writer) && !isTeman(IdCurrentUser, findIDPenggunaByName(&LPengguna,BALASAN(T).writer))){
